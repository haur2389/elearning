...
class LessonDetailView(generics.RetrieveAPIView):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        # Check access: preview lessons, enrolled students, instructor (owner), or admin
        if not lesson.is_preview:
            ...
            # Instructor có thể xem bất kỳ bài học nào (để preview khóa học của mình hoặc chuẩn bị giảng dạy)
            if not (is_enrolled or is_owner or is_admin or is_instructor):
...
class LessonCreateView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        notify_enrolled_students(
            ...
            message=f'Khóa học "{course.title}" vừa có bài học mới: {lesson.title}',
            notif_type='new_lesson',
            link=f'/learn.html?course={course.id}',
        )
