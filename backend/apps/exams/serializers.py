...
class ExamDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Exam
        # FIX: thêm 'course' — frontend (exam.html) cần course id sau khi nộp
        # bài để gọi API cấp chứng chỉ (issue-certificate) mà không có id này
        # thì không biết cấp chứng chỉ cho khóa học nào.
...
class ExamCreateSerializer(serializers.ModelSerializer):
    class Meta:
        model = Exam
        # FIX: thêm 'id' (read_only) — thiếu field này khiến response tạo đề thi
        # không trả về ID, làm frontend không thể gắn câu hỏi vào đúng đề thi.
        fields = ['id', 'course', 'title', 'description', 'duration_minutes',
